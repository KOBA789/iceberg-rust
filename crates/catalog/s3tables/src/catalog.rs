...
use aws_sdk_s3tables::operation::create_namespace::CreateNamespaceError;
use aws_sdk_s3tables::operation::create_table::{CreateTableError, CreateTableOutput};
...
impl Catalog for S3TablesCatalog {
    ...
    ///
    /// - If the namespace already exists, identified by the
    /// `NamespaceAlreadyExists` variant.
    ...
    async fn create_namespace(
        ...
        req.send().await.map_err(|err| {
            if err
                .as_service_error()
                .is_some_and(CreateNamespaceError::is_conflict_exception)
            {
                // The existence check above cannot rule this out: a concurrent
                // caller may create the namespace in between.
                Error::new(
                    ErrorKind::NamespaceAlreadyExists,
                    format!("Namespace {namespace:?} already exists"),
                )
    ...
    /// `DataInvalid` variant.
    /// - If the table already exists, identified by the `TableAlreadyExists`
    /// variant.
    /// - Errors from the underlying database creation process, converted using
    /// `from_aws_sdk_error`.
    async fn create_table(
        ...
        let create_resp: CreateTableOutput = self
            ...
            .map_err(|err| {
                if err
                    .as_service_error()
                    .is_some_and(CreateTableError::is_conflict_exception)
                {
                    Error::new(
