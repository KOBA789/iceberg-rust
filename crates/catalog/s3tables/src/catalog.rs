...
impl Catalog for S3TablesCatalog {
        ...
        req.send().await.map_err(|err| {
            ...
            match &err {
                SdkError::ServiceError(e) => match e.err() {
                    CreateNamespaceError::ConflictException(_) => Error::new(
                        ErrorKind::NamespaceAlreadyExists,
                        format!("Namespace {namespace} already exists"),
                    )
                    .with_source(anyhow::Error::msg(format!("aws sdk error: {err:?}"))),
                    _ => from_aws_sdk_error(err),
                },
                _ => from_aws_sdk_error(err),
            }
        })?;
    ...
    async fn create_table(
        ...
        let create_resp: CreateTableOutput = self
            ...
            .map_err(|err| {
                use aws_sdk_s3tables::error::SdkError;
                use aws_sdk_s3tables::operation::create_table::CreateTableError;
                match &err {
                    SdkError::ServiceError(e) => match e.err() {
                        CreateTableError::ConflictException(_) => Error::new(
